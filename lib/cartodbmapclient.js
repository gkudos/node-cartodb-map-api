...
class CartoDBMapClient {
  constructor(user, apiKey, baseURL = undefined) {
    this.user = user;
    this.apiKey = apiKey;
    
    if(baseURL) {
      baseURL = baseURL.endsWith('/')? baseURL:`${baseURL}/`;
      baseURL = baseURL.indexOf(user) != -1? baseURL:`${baseURL}user/${user}/`;
      this.baseURL = baseURL;
    } else {
      this.baseURL = 'https://' + this.user + '.carto.com/';
    }
  }
  ...
  put(suffixUrl, params, body, options) {
    var $this = this;
    if(!params) {
      params = {};
    }
  ...
  delete(suffixUrl, params, body, options) {
    var $this = this;
    if(!params) {
      params = {};
    }
